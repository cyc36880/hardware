...
void LSM6DS3TRC_ReadCommand(uint8_t reg_addr, uint8_t *rev_data, uint8_t length)
{
	if (lsm6ds3trc_mode == LSM6DS3TRC_MODE_I2C)
		I2C_ReadCommand(LSM6DS3TRC_I2CADDR, reg_addr, rev_data, length);
	else
		SPI_ReadCommand(reg_addr | 0x80, rev_data, length);
}
...
{
	if (lsm6ds3trc_mode == LSM6DS3TRC_MODE_I2C)
		I2C_WriteCommand(LSM6DS3TRC_I2CADDR, reg_addr, send_data, length);
	else
		SPI_WriteCommand(reg_addr & 0x7f, send_data, length);
}
...
bool LSM6DS3TRC_Init(uint8_t mode)
{
	...
	if (lsm6ds3trc_mode == LSM6DS3TRC_MODE_SPI)
	{
		spi.CPOL = 1;
		spi.CPHA = 1;
		spi.cs_status = 0;
