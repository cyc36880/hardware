...
#define LSM6DS3TRC_MODE_I2C 1
#define LSM6DS3TRC_MODE_SPI 2

#define LSM6DS3TRC_ADDRESS 0x6a
...
public:
	...
	boolean begin();
	uint8_t Pin_CS;
	uint8_t LSM6DS3TRC_MODE;
	uint8_t Get_Status(void);
	float Get_Temperature(void);
	void Get_Acceleration(uint8_t fsxl, float *acc_float);
	void Get_Gyroscope(uint8_t fsg, float *gry_float);

private:
	uint8_t _i2caddr;
	boolean GetChipID(void);
	void Reset(void);
	void Set_BDU(boolean flag);
	void Set_Accelerometer_Rate(uint8_t rate);
	...
	void Set_Register4(uint8_t reg4);
	void Set_Register5(uint8_t reg5);
	void Set_Register6(uint8_t reg6);
	void Set_Register7(uint8_t reg7);
	boolean ReadCommand(uint8_t reg_addr, uint8_t *rev_data, uint8_t length);
	void WriteCommand(uint8_t reg_addr, uint8_t *send_data, uint16_t length);
	boolean i2c_ReadCommand(uint8_t reg_addr, uint8_t *rev_data, uint8_t length);
	void i2c_WriteCommand(uint8_t reg, uint8_t *cmd, uint8_t length);
	void spi_ReadCommand(uint8_t reg, uint8_t *rev_data, uint8_t length);
	void spi_WriteCommand(uint8_t reg, uint8_t *send_data, uint8_t length);
};
