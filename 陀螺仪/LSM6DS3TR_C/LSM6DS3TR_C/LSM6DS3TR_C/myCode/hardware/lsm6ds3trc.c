...
void LSM6DS3TRC_ReadCommand(uint8_t reg_addr, uint8_t *rev_data, uint8_t length)
{
	...
	dev->Devive_IIC_readReg(dev, reg_addr, rev_data, length);
...
{
	IIC_DEVICE *dev = get_IIC_Port_DevHandle(1);
	dev->Devive_IIC_writeReg(dev, reg_addr, send_data, length);
...
bool LSM6DS3TRC_GetChipID(void)
{
	...
	printf("LSM6DS3TRC_GetChipID:%d\n", buf);
	if (buf == LSM6DS3TRC_I2CADDR)
		return true;
	else
		return false;
}
